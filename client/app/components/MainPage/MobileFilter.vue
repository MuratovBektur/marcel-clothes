          ...
          <div v-if="sheetOpen" class="sheet">
            ...
            <div class="sheet__section">
              <div class="sheet__section-label">
                Тип
                <button v-if="selectedType" class="sheet__section-clear"
                  @click="$emit('filter-type', '')">Сбросить</button>
              </div>
              <div class="sheet__chips">
                <button v-for="t in types" :key="t" class="sheet__chip" :class="{ active: selectedType === t }"
            ...
            <div class="sheet__section">
              ...
              <div class="sheet__price">
                <div class="sheet__price-field">
                  ...
                  <input v-model="localPriceMin" type="number" min="0" placeholder="0" class="sheet__price-input"
                    :class="{ 'sheet__price-input--error': priceError }" @change="applyPrice" />
                </div>
                <div class="sheet__price-sep">—</div>
                <div class="sheet__price-field">
                  <label class="sheet__price-label">До</label>
                  <input v-model="localPriceMax" type="number" min="0" placeholder="∞" class="sheet__price-input"
                    :class="{ 'sheet__price-input--error': priceError }" @change="applyPrice" />
                </div>
              </div>
            ...
            <div class="sheet__section">
              <div class="sheet__section-label">
                Материал
                <button v-if="selectedMaterials.length" class="sheet__section-clear"
                  @click="$emit('filter-materials', [])">Сбросить</button>
              </div>
              <div class="sheet__chips">
                <button v-for="m in materials" :key="m" class="sheet__chip"
            ...
            <div class="sheet__section">
              <div class="sheet__section-label">
                Цвет
                <button v-if="selectedColors.length" class="sheet__section-clear"
                  @click="$emit('filter-colors', [])">Сбросить</button>
              </div>
              <div class="sheet__chips">
                <button v-for="c in colors" :key="c" class="sheet__chip" :class="{ active: selectedColors.includes(c) }"
            ...
            <div class="sheet__section">
              <div class="sheet__section-label">
                Размер
                <button v-if="selectedSizes.length" class="sheet__section-clear"
                  @click="$emit('filter-sizes', [])">Сбросить</button>
              </div>
              <div class="sheet__chips">
                <button v-for="s in sizes" :key="s" class="sheet__chip" :class="{ active: selectedSizes.includes(s) }"
...
.sheet-overlay-fade-leave-active {
  transition: opacity 0.25s ease;
}

.sheet-overlay-fade-enter-from,
.sheet-overlay-fade-leave-to {
  opacity: 0;
}
